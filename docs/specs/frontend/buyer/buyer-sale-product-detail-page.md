...
- 구매하기 버튼은 UI만 제공하며 주문 요청, 화면 이동, modal 또는 다른 동작을 수행하지 않는다.
...
상세 API의 `quantity`는 판매 일정 등록 당시의 최초 판매 수량이며 남은 재고가 아니다. API에는 잔여 재고, `SOLD_OUT`과 구매 가능 여부가 없으므로 화면은 수량을 표시하거나 구매 가능 상태를 추론하지 않는다.
...
구매하기 버튼은 Obsidian 배경과 Paper White 글자의 pill 형태 Primary CTA로 표시한다. `button type="button"`을 사용하지만 event handler를 연결하지 않는다. 상태와 관계없이 문구는 `구매하기`로 유지하며 클릭이나 키보드 활성화 후에도 API 요청, route 변경, modal, 메시지 또는 상태 변경이 발생하지 않는다.

이 버튼은 후속 구매 흐름을 위한 시각적 경계일 뿐이다. 현재 응답만으로 구매 가능 여부를 판단하거나 비활성 상태를 추론하지 않는다.
...
- 구매하기 버튼 클릭이 API 요청, route 변경, modal 또는 상태 변경을 일으키지 않는지 확인한다.
- 긴 상품명, 설명과 여러 이미지에 대한 반응형 배치는 build와 실제 브라우저에서 확인한다.

## 주요 결정과 후속 작업

상단은 대표 이미지와 핵심 판매 정보만 제공하고, 상품 설명과 추가 이미지는 하단의 긴 세로 흐름으로 분리한다. 상세 식별자에는 판매 일정의 의미를 보존하는 `saleId`를 사용한다. 구매하기 버튼은 후속 주문 기능이 연결될 위치를 보여주는 UI 경계로만 제공한다.

후속 구매 기능은 인증, 잔여 재고의 원천, 구매 가능 상태, 주문 생성 실패와 중복 요청 정책을 별도로 설계한 뒤 이 버튼에 연결한다. 해당 설계가 승인되기 전에는 현재 버튼의 동작이나 상태 의미를 확장하지 않는다.
