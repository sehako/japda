...
[Google 소셜 로그인과 JWT 발급](google-social-login.md) 및 [ADR-020](../../../architecture/decisions/ADR-020-backend-google-oidc-login-and-jwt-cookie.md)의 후속 단계다. 로그인 성공 시 발급한 JWT에는 내부 사용자 ID가 `sub`로 들어가고 역할은 들어가지 않는다. JWT는 `JAPDA_ACCESS_TOKEN`이라는 `HttpOnly`, `SameSite=Lax`, `Path=/api` 쿠키에 담기며 유효기간은 1시간이다. `users`에는 ID와 이메일이, `user_roles`에는 `BUYER`·`ADMIN`이 저장된다.
...
`GET /api/auth/me`에 한정된 Spring Security 인증 경계를 구성하고, 다른 요청에는 기존 Google OAuth2 Login 설정과 `permitAll` 계약을 유지한다. `AuthenticationConverter`가 `JAPDA_ACCESS_TOKEN` 쿠키만 읽어 인증 전 `Authentication`을 만들고, 기존 Spring Security의 `AuthenticationFilter`가 이를 JWT 전용 `ProviderManager`에 전달한다. 이 관리자는 해당 필터에만 연결해 Google OAuth2 Login의 인증 제공자 설정을 변경하지 않는다. JWT 전용 `AuthenticationProvider`는 기존 의존성의 `NimbusJwtDecoder`로 JWT를 검증한 뒤 내부 사용자 ID를 principal로 하고 원문 JWT를 credentials에 남기지 않는 인증 결과를 반환한다. 필터는 인증 결과를 현재 요청의 `SecurityContext`에 설정하되 세션에 저장하지 않으며, 성공 핸들러는 리다이렉트 없이 요청을 `/api/auth/me`로 계속 전달한다. 별도의 JWT 필터를 직접 상속해 만들거나 Resource Server starter, JJWT, 비밀번호 로그인용 `DaoAuthenticationProvider`를 추가하지 않는다.

JWT 검증에는 기존 발급 설정과 같은 서명 키를 사용하며, 허용 알고리즘은 `HS256`으로 제한한다. 설정된 발급자·대상과 필수 `exp`를 확인하고 `exp`가 현재 시각 이하인 토큰은 거부한다. `sub`는 양의 내부 사용자 ID로 해석한다. 쿠키가 없으면 `AuthenticationConverter`가 인증 시도를 만들지 않고 `/api/auth/me`의 인증 요구에서 `401`을 반환한다. 쿠키가 있지만 검증에 실패하면 `AuthenticationProvider`가 인증 실패를 알린다. 두 경로 모두 Spring Security의 인증 실패 처리에서 동일한 `AUTH_UNAUTHENTICATED` `ProblemDetail`과 `Cache-Control: no-store`로 변환한다. Google OAuth2 세션의 인증 정보나 클라이언트가 임의로 보낸 ID 헤더는 이 API의 신원 근거로 사용하지 않는다.

`auth.presentation`은 인증된 사용자 ID를 받아 HTTP 응답을 처리한다. `auth.application`은 `UserRepository`와 `UserRoleRepository`를 통해 사용자와 역할을 읽고 응답 객체를 조립한다. JPA 조회 구현은 `auth.infrastructure.persistence`에 둔다. Entity를 직접 반환하거나 JWT의 역할 claim에서 응답을 구성하지 않는다. 사용자가 DB에 없으면 application 조회 결과를 `401`로 변환하고, DB 장애는 서버 오류로 유지한다. 조회는 상태를 변경하지 않으며 새 DB 테이블이나 migration을 추가하지 않는다.
...
- `AuthenticationConverter`와 JWT `AuthenticationProvider` 테스트에서 쿠키 추출, 정상 서명, 변조, 만료와 `exp` 누락, 잘못된 발급자·대상·알고리즘·`sub`를 확인한다. 인증 성공 후 현재 요청에만 `SecurityContext`가 설정되고 원문 JWT가 결과 credentials에 남지 않는지도 확인한다.
- 사용자 조회 테스트에서 DB 이메일 변경과 역할 추가·제거가 다음 조회에 반영되는지 확인한다.
- MockMvc와 Spring REST Docs에서 성공 응답의 필드·헤더와 쿠키 누락·무효·삭제된 사용자에 대한 동일한 `401 ProblemDetail`을 검증한다. Google 세션과 ID 헤더만 있는 요청도 `401`인지 확인한다.
- 명시적 SPA origin의 자격 증명 CORS 요청을 검증하고, 기존 도메인 API의 접근·헤더 계약 회귀 테스트 및 백엔드 `build`를 실행한다. 실제 Google 계정이나 운영 비밀값은 사용하지 않는다.

기존 [ADR-020](../../../architecture/decisions/ADR-020-backend-google-oidc-login-and-jwt-cookie.md)은 JWT 발급까지만 결정했다. 이번 쿠키 JWT 인증 경계와 현재 의존성의 Spring Security 구성 요소를 사용하는 결정은 [ADR-021](../../../architecture/decisions/ADR-021-cookie-jwt-authentication-with-spring-security-components.md)에 기록한다.
