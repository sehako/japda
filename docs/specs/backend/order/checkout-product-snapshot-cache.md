...
구매자가 `GET /api/checkout`으로 주문 직전 정보를 확인할 때, 판매별로 불변인 상품 표시 정보와 판매 단가의 반복 조회 병목을 줄인다. Redis에 판매별 상품 스냅샷을 저장하고, fresh 기간이 지난 값은 stale-while-revalidate(SWR) 방식으로 즉시 반환하면서 비동기 재적재한다.

다음 조건을 모두 만족하면 완료로 본다.

- 상품명, 대표 이미지 경로, 판매 단가를 `saleId`별 스냅샷으로 Redis에 캐시한다.
- 배송지와 그 밖의 구매자별 정보는 캐시하지 않고 요청마다 PostgreSQL에서 조회한다.
- 캐시 미스와 hard-expire의 동시 요청은 각각 원본 DB를 조회할 수 있다.
- stale 기간에는 이전 스냅샷을 응답하고 비동기 재적재를 시작한다.
...
포함 범위는 체크아웃 상품 스냅샷 캐시, SWR, 캐시 장애 우회, 관측과 테스트다. `GET /api/checkout`의 URL·요청·응답 형식, 주문 생성, 재고 예약, 결제, 배송지 CRUD, 상품·판매 수정 및 삭제 API는 변경하지 않는다.
...
| stale | 적재 후 5분 초과~60분 이하 | 캐시 값을 즉시 반환하고 비동기 재적재를 시작한다. |
| hard-expire | 적재 후 60분 초과 | 캐시 값을 사용하지 않고 원본 DB에서 조회해 적재한다. |
...
캐시 미스 또는 hard-expire에서 요청은 PostgreSQL 원본에서 상품 스냅샷을 조회하고 Redis에 fresh·stale 만료 시각과 물리 TTL을 포함해 저장한다. 동시에 들어온 요청의 중복 원본 조회와 저장은 허용한다. 불변 상품 스냅샷의 단순 읽기 비용보다 분산 lock, polling, 소유 토큰 관리의 복잡도가 크다고 판단한다.

stale 값이 있으면 요청은 값을 즉시 반환하고 별도 실행기에서 비동기 재적재를 시작한다. 재적재 실패 시 stale 값은 `staleUntil`까지 유지하며, 다음 stale 요청도 다시 재적재를 시작할 수 있다.

## 장애 처리와 관측

Redis 연결·명령 오류, 직렬화·역직렬화 오류, 비동기 재적재 오류는 오류 로그와 메트릭을 남긴다. 동기 요청에서는 원본 PostgreSQL 조회로 우회하고, 비동기 재적재 오류는 사용자 응답에 영향을 주지 않는다. PostgreSQL 원본 조회 실패와 기존의 상품·대표 이미지 불변식 위반은 현재 체크아웃 오류 계약대로 처리한다.
