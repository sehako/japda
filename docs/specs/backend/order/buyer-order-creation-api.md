...
- 생성된 주문은 `PENDING_PAYMENT` 상태로 10분 동안 판매 수량을 예약한다.
...
포함 범위는 `POST /api/orders`, 단일 판매 상품 주문, 배송 정보 스냅샷, 가격 계산, 10분 재고 예약, 판매 일정 잠금, 멱등성 처리, 오류 응답, PostgreSQL 테스트와 API 문서이다.
...
`totalPrice`는 `Sale.price × quantity`이며 `Long` 범위를 넘지 않는지 검사한다. `expiresAt`은 주문 생성에 사용한 UTC `Instant`에서 10분을 더한 값이다. 배송 정보는 저장하지만 결제 시작에 필요한 성공 응답에는 반환하지 않는다.
...
- 주문 만료 기간 10분은 의미가 드러나는 주문 설정값으로 관리하며 코드 곳곳에 숫자 리터럴을 반복하지 않는다.
...
- 생성 상태가 `PENDING_PAYMENT`이고 `expiresAt`이 생성 시각에서 정확히 10분 뒤인지 확인한다.
