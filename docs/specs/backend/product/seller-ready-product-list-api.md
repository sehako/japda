...
판매 일정 등록에 사용할 수 있도록 판매자가 본인 소유의 `READY` 상품 목록을 조회하는 API를 제공한다. 목록은 상품 식별자, 상품명과 판매자에게 같은 이름의 READY 상품이 있는지를 반환하고, 상품 수가 증가해도 일정한 조회 비용을 유지할 수 있도록 정렬 기준별 커서 페이지네이션을 적용한다.

다음 조건을 모두 만족하면 완료된 것으로 본다.

- 판매자는 본인 소유이면서 `READY` 상태인 상품만 조회할 수 있다.
- 응답 항목은 상품 식별자, 상품명과 동명 상품 여부를 포함한다.
- 현재 페이지 밖의 상품까지 포함해 같은 판매자의 READY 상품명이 중복되는지 판별한다.
...
- 동명 상품 여부를 포함한 조회 전용 응답과 projection
- PostgreSQL 조회 인덱스
...
- `hasDuplicateName`은 같은 판매자의 다른 `READY` 상품 중 응답 항목과 상품명이 정확히 같은 상품이 하나 이상 있으면 `true`다. 현재 응답 페이지 밖에 있는 상품도 판별 대상에 포함한다.
- 다른 판매자의 상품, `DRAFT` 상품과 자기 자신은 동명 상품 판별에서 제외한다.
...
SELECT p.id,
       ...
       EXISTS (
           ...
           WHERE duplicate_product.seller_id = p.seller_id
             AND duplicate_product.status = 'READY'
             AND duplicate_product.name COLLATE "C" = p.name COLLATE "C"
...
조회에서는 전체 `Product` Entity 대신 `id`, `name`, `hasDuplicateName`만 선택하는 전용 projection을 사용한다. `hasDuplicateName`은 같은 `seller_id`, `READY` 상태와 `C` collation 기준으로 정확히 같은 `name`을 가지면서 `id`가 다른 상품의 존재 여부를 상관 `EXISTS`로 구한다. 전체 동명 상품 수는 세지 않으며 `products_seller_ready_name_id_idx`로 존재 여부를 확인한다. 상품 설명, 생성 시각을 불필요하게 로딩하지 않고 `sales`나 `product_images`와 조인하지 않는다.
...
- Repository가 반환한 동명 상품 여부를 각 응답 항목에 포함하는지 검증한다.
...
- 응답 항목의 `hasDuplicateName` 필드를 필수 boolean으로 검증하고 문서화한다.
...
- 같은 판매자의 다른 READY 상품과 이름이 같을 때만 `hasDuplicateName`이 `true`인지 검증한다.
- 동명 상품이 다른 응답 페이지에 있더라도 양쪽 항목의 `hasDuplicateName`이 `true`인지 검증한다.
- 다른 판매자, `DRAFT` 상품과 자기 자신은 동명 상품 판별에서 제외되는지 검증한다.
...
- 프런트엔드가 일반 상품에는 내부 식별자를 노출하지 않고 동명 상품만 구분할 수 있도록 `hasDuplicateName`을 항상 응답한다.
