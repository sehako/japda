...
- 집중되는 주문의 품절 요청을 DB 접근 전에 거절하기 위해 실제 주문이 발생한 판매 일정의 가용 재고만 Redis에 30초 동안 동적으로 적재한다. JVM 내부 상태를 공유하지 않고 모든 cache miss 요청이 Redis 분산 lock을 시도하는 Single-flight로 초기화를 제한하며, Lua script로 재고를 원자적으로 선점한다. 기존 판매 일정 잠금과 주문 예약 집계를 최종 정합성 기준으로 유지하고 Redis 장애는 기존 DB 주문 경로로 우회하며 TTL을 요청마다 연장하지 않는다. [ADR-026](decisions/ADR-026-dynamic-redis-inventory-reservation.md)과 이를 대체한 [ADR-027](decisions/ADR-027-redis-only-inventory-initialization-single-flight.md)을 따른다.
