...
- 집중되는 주문의 품절 요청을 DB 접근 전에 거절하기 위해 실제 주문이 발생한 판매 일정의 가용 재고만 Redis에 30초 동안 동적으로 적재한다. JVM 내부 상태를 공유하지 않고 모든 cache miss 요청이 Redis 분산 lock을 시도하는 Single-flight로 초기화를 제한하며, Lua script로 재고를 원자적으로 선점한다. Redis는 보조 선점 계층이며 장애 시 DB 주문 경로로 우회하고 TTL을 요청마다 연장하지 않는다. snapshot은 만료 예약을 반환한 뒤 `Sale.quantity - committed_quantity`로 생성하며 PostgreSQL 조건부 UPDATE를 최종 재고 정합성 기준으로 사용한다. [ADR-027](decisions/ADR-027-redis-only-inventory-initialization-single-flight.md)과 [ADR-028](decisions/ADR-028-database-inventory-counter-and-order-reservation.md)을 따른다.
