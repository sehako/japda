...
class AuthSecurityConfig {
    ...
    fun securityFilterChain(
        ...
        val csrfRepository = CookieCsrfTokenRepository().apply {
            ...
            setCookieCustomizer { cookie -> cookie.path("/api").httpOnly(true).sameSite("Lax").secure(properties.cookieSecure) }
        }

        val jwtFilter = AuthenticationFilter(
            ProviderManager(JwtCookieAuthenticationProvider(properties.signingKey(), properties.issuer, properties.audience, users)),
        ...
        http.authorizeHttpRequests { requests ->
            ...
            .exceptionHandling { exceptions -> exceptions.authenticationEntryPoint { request, response, _ -> unauthorized(request, response) }
                .accessDeniedHandler { request, response, _ ->
                    val error = if (org.springframework.security.core.context.SecurityContextHolder.getContext().authentication?.principal is Long)
        ...
        if (registrationRepository.getIfAvailable() != null) {
            ...
            http.oauth2Login { oauth ->
                oauth.successHandler(GoogleLoginSuccessHandler(loginService, issuer, properties.successUrl, properties.failureUrl, properties.cookieSecure, csrfRepository))
