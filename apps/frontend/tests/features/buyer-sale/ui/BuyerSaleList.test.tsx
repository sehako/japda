...
describe('구매자 판매 상품 목록', () => {
  test('loading, 빈 결과와 오류를 서로 다른 상태로 표시한다', () => {
    const { rerender } = render(<BuyerSaleList sales={undefined} isLoading isError={false} onRetry={vi.fn()} imageBaseUrl="" />)
    expect(screen.getByRole('status')).toHaveTextContent('판매 상품을 불러오는 중입니다.')

    rerender(<BuyerSaleList sales={[]} isLoading={false} isError={false} onRetry={vi.fn()} imageBaseUrl="" />)
    expect(screen.getByText('선택한 날짜에 판매 상품이 없습니다.')).toBeInTheDocument()

    const retry = vi.fn()
    rerender(<BuyerSaleList sales={undefined} isLoading={false} isError onRetry={retry} imageBaseUrl="" />)
    expect(screen.getByRole('alert')).toHaveTextContent('판매 상품을 불러오지 못했습니다.')
    fireEvent.click(screen.getByRole('button', { name: '다시 시도' }))
    expect(retry).toHaveBeenCalledOnce()
  })

  test('상품 순서를 유지하고 카드에는 이미지, 상태, 이름과 가격만 표시한다', () => {
    const ended = { ...sale, saleId: 12, productId: 22, name: '두 번째 상품', status: 'ENDED' as const }
    const { container } = render(<BuyerSaleList sales={[sale, ended]} isLoading={false} isError={false} onRetry={vi.fn()} imageBaseUrl="https://images.example.com/" />)
    ...
    expect(container.querySelector('a, article button')).toBeNull()
  ...
  test('이미지 설정이 없거나 이미지 로드가 실패하면 접근 가능한 대체 영역을 표시한다', () => {
    const { rerender } = render(<BuyerSaleList sales={[sale]} isLoading={false} isError={false} onRetry={vi.fn()} imageBaseUrl="" />)
    expect(screen.getByRole('img', { name: '한정판 후디 이미지 없음' })).toBeInTheDocument()

    rerender(<BuyerSaleList sales={[sale]} isLoading={false} isError={false} onRetry={vi.fn()} imageBaseUrl="https://images.example.com" />)
