...
describe('구매자 체크아웃 API', () => {
  test('구매자 식별자와 수량, AbortSignal을 전달한다', async () => {
    ...
    await expect(fetchBuyerCheckout(11, 3, 42, controller.signal, { baseUrl: 'http://localhost:8080', fetcher })).resolves.toEqual(checkout)
  ...
  test.each([
    ...
    await expect(fetchBuyerCheckout(11, 3, 42, undefined, { fetcher: async () => Response.json(body) })).rejects.toBeInstanceOf(ApiError)
  })

  test.each([0, -1, 1.5, Number.MAX_SAFE_INTEGER + 1])('잘못된 구매자 식별자 %s에서는 요청하지 않는다', async (buyerId) => {
    const fetcher = vi.fn<typeof fetch>()
    await expect(fetchBuyerCheckout(11, 3, buyerId, undefined, { fetcher })).rejects.toBeInstanceOf(ApiError)
    expect(fetcher).not.toHaveBeenCalled()
  })

  test('배송지 등록 요청에 구매자 식별자와 필드를 전달한다', async () => {
    let request: Request | undefined
    const fetcher: typeof fetch = async (input, init) => {
      request = new Request(input, init)
      return Response.json({ ...address, createdAt: '2026-09-13T00:00:00Z' }, { status: 201 })
    }
    await expect(createShippingAddress({ ...address, shippingAddressId: undefined }, 42, { baseUrl: 'http://localhost', fetcher })).resolves.toMatchObject(address)
  ...
  test('잘못된 등록 성공 응답을 거절한다', async () => {
    const fetcher = vi.fn<typeof fetch>(async () => Response.json({ ...address, shippingAddressId: null }, { status: 201 }))
    await expect(createShippingAddress({ ...address, shippingAddressId: undefined }, 42, { baseUrl: 'http://localhost', fetcher })).rejects.toBeInstanceOf(ApiError)
  })
})
