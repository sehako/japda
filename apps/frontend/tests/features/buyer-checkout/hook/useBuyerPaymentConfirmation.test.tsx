...
test('유효한 성공 정보로 한 번 승인하고 검증된 주문을 반환한다', async () => {
  ...
  const { result } = renderHook(() => useBuyerPaymentConfirmation(params, 123), { wrapper })

  expect(result.current.kind).toBe('confirming')
  await waitFor(() => expect(result.current).toEqual({ kind: 'paid', payment: paid }))
  expect(confirm).toHaveBeenCalledExactlyOnceWith(params, 123, { baseUrl: '' })
})

test('성공 정보 또는 구매자 설정이 없으면 승인 요청을 보내지 않는다', async () => {
  const { result, rerender } = renderHook(({ payment, buyerId }) => useBuyerPaymentConfirmation(payment, buyerId), {
    initialProps: { payment: null as TossSuccessParameters | null, buyerId: 123 as number | null }, wrapper,
  })
  expect(result.current.kind).toBe('invalid')
  rerender({ payment: params, buyerId: null })
  expect(result.current.kind).toBe('invalid')
...
test('StrictMode 재실행과 다시 렌더링에도 최초 승인 요청은 한 번만 보낸다', async () => {
  ...
  const { rerender } = renderHook(() => useBuyerPaymentConfirmation({ ...params }, 123), { wrapper: Wrapper })
...
test('화면을 떠나면 예정된 추가 확인을 실행하지 않는다', async () => {
  confirm.mockRejectedValue(new ApiError('처리 중', { status: 409, code: 'PAYMENT_CONFIRMATION_IN_PROGRESS' }))
  const { result, unmount } = renderHook(() => useBuyerPaymentConfirmation(params, 123), { wrapper })
  await waitFor(() => expect(result.current.kind).toBe('processing'))
  unmount()
  await act(async () => { await vi.advanceTimersByTimeAsync(75_000) })
  expect(confirm).toHaveBeenCalledTimes(1)
...
test('URL 결제가 바뀌면 이전 응답을 무시하고 새 결제만 표시한다', async () => {
  ...
  const { result, rerender } = renderHook(({ payment }) => useBuyerPaymentConfirmation(payment, 123), {
...
test('계약 오류와 네트워크 오류는 결과 확인 필요로 처리한다', async () => {
  ...
  const { result, rerender } = renderHook(({ payment }) => useBuyerPaymentConfirmation(payment, 123), { initialProps: { payment: params }, wrapper })
