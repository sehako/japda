...
test('상품 생성은 판매자 헤더 없이 쿠키와 CSRF로 정규화된 JSON을 전송한다', async () => {
  ...
  const result = await createProduct({ name: '상품', description: null }, undefined, { baseUrl: 'http://localhost', fetcher })

  assert.equal(result.id, 42)
  assert.equal(result.name, '상품')
  assert.equal(request && (request as Request).headers.has('X-Seller-Id'), false)
  assert.equal((request as unknown as Request).credentials, 'include')
  assert.equal((request as unknown as Request).headers.get('X-CSRF-TOKEN'), 'csrf-token')
...
test('상품 생성 성공 응답이 DRAFT 계약과 다르면 실패한다', async () => {
  const fetcher: typeof fetch = async (input) => String(input).endsWith('/api/auth/csrf')
    ...
    : Response.json({ id: 42, status: 'READY' }, { status: 201 })
  await assert.rejects(createProduct({ name: '상품', description: null }, undefined, { fetcher }), ApiError)
})
...
test('이미지 등록은 파일 순서와 대표 인덱스를 multipart로 전송하고 Content-Type은 직접 지정하지 않는다', async () => {
  ...
  await registerProductImages(42, [first, second], 1, undefined, { baseUrl: 'http://localhost', fetcher })
...
test('이미지 등록 성공 응답의 상품 식별자가 요청과 다르면 실패한다', async () => {
  const fetcher: typeof fetch = async (input) => String(input).endsWith('/api/auth/csrf')
    ? Response.json({ token: 'csrf-token', headerName: 'X-CSRF-TOKEN' })
    : Response.json({ productId: 99, status: 'READY', images: [] }, { status: 201 })
  await assert.rejects(registerProductImages(42, [new File(['a'], 'a.jpg')], 0, undefined, { fetcher }), ApiError)
})
